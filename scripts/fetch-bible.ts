/**
 * Script to download Bible data from getBible v2 API
 * and split into per-book JSON files.
 *
 * Usage: npx ts-node scripts/fetch-bible.ts
 */
...
// getBible v2 book keys mapped to our book IDs
...
async function fetchTranslation(translationKey: string) {
  const outDir = path.join(__dirname, '..', 'assets', 'bible', translationKey);
  ...
  for (const [bookNum, bookId] of Object.entries(BOOK_MAP)) {
    ...
    try {
      ...
      // Transform to { chapter: { verse: text } } format
      ...
      console.log(`  ${bookId} (${Object.keys(bookData).length} chapters)`);

      // Small delay to be respectful to the API
      await new Promise((r) => setTimeout(r, 200));
    } catch (err) {
      console.error(`  Error fetching ${bookId}:`, err);
    }
  }
